...
export default function RegisterScreen() {
    ...
    const handleRegistration = async () => {
        ...
        try {
            ...
            // This creates the record in the 'auth.users' table
            ...
            // 2. CREATE PROFILE in your 'e_print_users' table
            const { error: profileError } = await supabase
                .from('e_print_users') // matches your screenshot
                .insert([
                    {
                        id: authData.user.id,     // Primary Key (UUID)
                        student_id: studentId,   // matches your column
                        fullname: fullName,     // matches your column
                        email: email,           // matches your column
                        role: "student",        // matches your column
                        // password: password,  <-- STRONGLY RECOMMENDED TO LEAVE THIS OUT
                    },
                ]);

            if (profileError) {
                // If profile creation fails, we should handle it (maybe delete the auth user)
                throw profileError;
            }
            ...
            // Handle the "429 Rate Limit" error specifically if it happens
            if (error.message?.includes("rate limit")) {
                Alert.alert("Too many attempts", "Please wait a few minutes or disable 'Confirm Email' in Supabase settings.");
            } else {
