...
export default function OrdersScreen() {
    ...
    const fetchOrders = async () => {
        try {
            ...
            // 2. Query 'print_jobs' table where user_id matches
            // We order by 'created_at' descending so the newest jobs appear first
    ...
    const renderOrderItem = ({ item }: { item: Order }) => {
        ...
        return (
            <View style={styles.orderCard}>
                ...
                {/* Note: In a real app, you might join with print_shops table to get the name */}
                <Text style={styles.orderShopName}>Shop ID: {item.print_shop_id}</Text>
    ...
    // ... (rest of the component remains the same for UI/Loading)
    if (loading) {
        return (
    ...
    return (
        <View style={{ flex: 1, backgroundColor: "#FAFAFB" }}>
            <FlatList
                ...
                renderItem={renderOrderItem}
                contentContainerStyle={[styles.homePadding, { paddingBottom: 100 }]}
                refreshControl={
                    <RefreshControl refreshing={refreshing} onRefresh={onRefresh} />
                }
                ListHeaderComponent={() => (
                ...
                ListEmptyComponent={() => (
                    <View style={{ alignItems: "center", marginTop: 80, paddingHorizontal: 40 }}>
                        <Ionicons name="document-text-outline" size={60} color="#DDD" />
                        <Text style={{ fontSize: 18, fontWeight: "600", color: "#888", marginTop: 15 }}>
                            No orders yet
                        </Text>
                        <Text style={{ color: "#AAA", textAlign: "center", marginTop: 8 }}>
                            You haven't submitted any print jobs. Tap 'Submit' to get started!
            ...
            {/* Nav component remains as you provided */}
            <View style={styles.bottomNav}>
